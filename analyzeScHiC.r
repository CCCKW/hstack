...
source("/Users/ckw/warehouse/metacell/stark/map3c/TG3C/params.r")
source("/Users/ckw/warehouse/metacell/stark/map3c/TG3C/PipeStats.r")
source("/Users/ckw/warehouse/metacell/stark/map3c/TG3C/analyzeHiC.r")
...
sch_load_config <- function(spec_params_fn) {
  ...
  sch_data_dir <<- "/Users/ckw/warehouse/metacell/stark/config/"
  ...
  # colors
  twenty_colors <<- c(
    "cadetblue", "lightsalmon", "mediumseagreen", "cornflowerblue", "bisque3",
  ...
  if (dir.exists(sch_groot)) {
    ...
    warning(sprintf("Skipping gdb.init: database directory does not exist: %s", sch_groot))
  }
}
...
sch_create_chrom_stat_marg_and_z <- function() {
  ...
  res <- eval(parse(text = paste("gcluster.run(", commands, ")")))
  chr_counts <- do.call("rbind", lapply(res, function(x) {
    x$retv
  }))

  n_chroms <- nrow(gintervals.all())
  rownames(chr_counts) <- nms
  colnames(chr_counts) <- paste(
    c(
      rep("self", n_chroms), rep("2M", n_chroms),
...
.sch_create_fend_dup_nm <- function(nm, ints) {
  ...
  tapply(x$start1, as.character(x$chrom1), function(y) {
    z <- table(y)
    sum(z > 1) / length(z)
  })
}
...
sch_create_repli_stat <- function(n_partitions = 10, chroms = sch_chroms) {
  ...
  tor_counts <- array(0, dim = c(n_partitions, length(nms), n_chroms * 8), dimnames = list(part = 1:10, nm = nms, mode = paste(
    c(
      rep("self_tor1", n_chroms), rep("self_tor2", n_chroms),
...
sch_report_karyo_and_qc <- function(trans_trim_to = NULL) {
  ...
  abline(
    v = 1 - length(bad_cells_thresh) / nrow(sch_chrom_marg_z),
    col = "black", lwd = 2
  )
  dev.off()
  ...
  for (col in self_cols[-1]) {
    plot(chr_self[, self_cols[1]], chr_self[, col],
      pch = 21,
...
.sch_plot_group_mean_mat <- function(base_dir, nms, no_dig_dist = 1000, w_per_100m = 200, dens_white_quantile = 0.1, scope = gintervals.2d.all(), force_size = -1, bwx = 1e5, bwy = bwx, fn_pref = "", ngrid = 1000, shades = colorRampPalette(c("white", "white", "white", "white", "white", "white", "white", "white", "white", "white", "gray", "gray", "lightblue", "blue", "darkblue", "orange", "yellow", "black")), name = "", type = "kde", add_axis = F, point_cex = 0.5, rotate = F, glob_dens_lim = NULL, ablines_x = NULL, ablines_y = NULL, ablines_lwd = 1) {
  ...
  for (chr_nm in unique(scope$chrom1)) {
    ...
    if (type == "kde") {
      ...
      image(d2d$x1, d2d$x2, t(log10(dens)),
        col = shades(1000),
...
sch_create_pooled_track_wrapper <- function(nms, pool_nm) {
  if (haploid) {
    ...
    all_dirs <- unlist(sapply(sch_base_dir, function(x) {
      paste(x, all_names, sep = "/")
    }))
...
.sch_get_pool_tad_borders <- function(iterator = 1e3, input_track = pool_tn, ins_track = ins_tn, scale = ins_scale, min_diag = sch_remove_near_dist, filter_by_cov_per_kb = 50, filter_iter = 0, scope = NULL) {
  ...
  bp <- do.call(rbind, lapply(split(bords, bords$intervalID), function(x) {
    x[which.min(x$ins)[1], ]
  }))
...
plot_trans_helper <- function(nm, chroms, cex = 0.3, alpha = 0.5, bp_per_px = 5e5, odir = NULL, grid_every_bp = 20e6) {
  ...
  if (grid_every_bp > 0) {
    grids <- do.call("c", sapply(rownames(chrs), function(chr) {
      seq(chrs[chr, "glob_start"], chrs[chr, "glob_end"], by = grid_every_bp)
    }))
...
count_dup_trans_contacts_in_spec_chroms_across_all_cells <- function(chr1 = "chr15", chr2 = "chr16") {
  ...
  tab <- table(do.call("c", lapply(nms, function(nm) {
    x <- gextract(nm, intervals = ints)
    if (!is.null(x)) {
      message(paste(nm, nrow(x)))
      paste(x$start1, x$start2)
    }
  })))
...
compute_reads_per_contact <- function() {
  ...
  res <- eval(parse(text = paste("gcluster.run(", commands, ")")))
  stopifnot(sum(unlist(lapply(res, function(r) {
    typeof(r$retv) == "list"
  }))) == length(res))
