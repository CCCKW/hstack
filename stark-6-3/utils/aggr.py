...
def aggregate_metacell_mat_EM(
    ...
    """
    Metacell 内一致性加权聚合（EM / Iterative Reweighting）——稀疏加速版。

    对每个 Metacell 内部执行若干轮 EM 迭代：
      E 步：用当前权重加权求和，得到 prototype P_m
      M 步：计算每个细胞矩阵与 P_m 的全局余弦相似度，经 softmax 归一化后更新权重

    偏离 metacell 主体结构的"异质细胞"会被逐步降权，从而使最终聚合更加干净。

    速度优化
    ----------
    全程保持稀疏格式，不调用 .toarray()：
      - 每个 chrom 的 n_cells 个细胞矩阵 vstack 成 (n_cells, B²) CSR 稀疏矩阵
      - E 步: proto_flat = weights @ stacked  (稀疏 matvec)
      - M 步: dot_products = stacked @ proto_flat  (稀疏 matvec)
      - 细胞范数预计算一次，跨 EM 迭代全部复用

    参数
    ------
    hdata        : HData 对象，需已完成 process_and_load 与 fit。
    n_iter       : EM 迭代次数，默认 3。
    temperature  : Softmax 温度参数（越小差异越大），默认 1.0。
    force_aggregate : 若为 True，则忽略已有结果强制重算。
    verbose      : 是否打印进度信息。

    结果
    ------
    聚合结果（稀疏矩阵）储存于
        hdata.metacell_data['mat_EM'][str(resolution)][m_id][chrom]
    格式与 aggregate_metacell_mat 完全一致。
    """
    import scipy.sparse as sp
    ...
    if 'mat_EM' not in hdata.metacell_data:
        hdata.metacell_data['mat_EM'] = {}

    metacell_groups = hdata.obs.groupby('metacell').groups
    chrom_list = hdata.chrom_list

    # 预计算细胞位置索引（与分辨率无关，只算一次）
    pos_idx_map = {
        m_id: [hdata.obs.index.get_loc(idx) for idx in indices_list]
        for m_id, indices_list in metacell_groups.items()
    }
    ...
    for resolution, mat_dict in hdata.views_mat.items():
        ...
        if str_res in hdata.metacell_data['mat_EM'] and not force_aggregate:
            if verbose:
                print(f"⏭️  分辨率 {resolution} EM 聚合结果已存在，跳过 (使用 force_aggregate=True 强制重跑)。")
            completed_resolutions.append(resolution)
            continue
        ...
        hdata.metacell_data['mat_EM'][str_res] = {}
        all_res_dict = hdata.metacell_data['mat_EM'][str_res]

        for m_id, pos_indices in tqdm(pos_idx_map.items(), desc=f"EM Aggr @ {resolution}"):
            n_cells = len(pos_indices)

            # ============================================================
            # Step 1: 预计算 stacked CSR 稀疏矩阵 & 细胞范数（只做一次）
            # stacked[chrom]: (n_cells, B²) CSR，不调用 toarray()，保持稀疏
            # ============================================================
            stacked = {}     # chrom -> (n_cells, B²) CSR
            bin_shapes = {}  # chrom -> (B, B)
            cell_norms = np.zeros(n_cells, dtype=np.float64)

            for chrom in chrom_list:
                rows = []
                for idx in pos_indices:
                    m = mat_dict[chrom][idx]
                    m_csr = (m if sp.issparse(m) else sp.csr_matrix(m)).astype(np.float64)
                    rows.append(m_csr.reshape(1, -1))   # (1, B²) 稀疏行
                bin_shapes[chrom] = mat_dict[chrom][pos_indices[0]].shape
                stacked_c = sp.vstack(rows, format='csr')  # (n_cells, B²)
                stacked[chrom] = stacked_c
                # 每行范数²：只碰非零元，无需 toarray
                cell_norms += np.asarray(stacked_c.power(2).sum(axis=1)).ravel()

            cell_norms = np.sqrt(cell_norms)  # (n_cells,)

            # ============================================================
            # Step 2: EM 迭代（全稀疏 matvec，无大规模内存分配）
            # ============================================================
            weights = np.ones(n_cells, dtype=np.float64) / n_cells

            for _it in range(n_iter):
                # E 步: weights(n_cells,) @ stacked(n_cells,B²) → proto_flat(B²,) dense
                proto_norm_sq = 0.0
                proto_flats = {}
                for chrom in chrom_list:
                    pf = weights @ stacked[chrom]  # scipy 稀疏 matvec，只碰非零元
                    proto_flats[chrom] = pf
                    proto_norm_sq += float(np.dot(pf, pf))

                proto_norm = np.sqrt(proto_norm_sq)
                if proto_norm < 1e-12:
                    break

                # M 步: stacked(n_cells,B²) @ proto_flat(B²,) → dot_products(n_cells,)
                dot_products = np.zeros(n_cells, dtype=np.float64)
                for chrom in chrom_list:
                    dot_products += stacked[chrom] @ proto_flats[chrom]

                # softmax 重加权
                safe_norms = np.where(cell_norms < 1e-12, 1.0, cell_norms)
                cosine_sim = dot_products / (safe_norms * proto_norm)
                cosine_sim = np.where(cell_norms < 1e-12, 0.0, cosine_sim)
            ...
            # ============================================================
            # Step 3: 最终聚合 → reshape 回 (B,B)，存为 float32 稀疏矩阵
            # ============================================================
            m_id_dict = {}
            for chrom in chrom_list:
                pf = weights @ stacked[chrom]  # (B²,) dense
                B = bin_shapes[chrom][0]
                m_id_dict[chrom] = sp.csr_matrix(pf.reshape(B, B), dtype=np.float32)
    ...
    if verbose:
        print(f"\n✅ EM 聚合完成！分辨率: {sorted(completed_resolutions)}")
        if completed_resolutions:
            print(
                f"   调用示例: sk.pl.plot_metacell_heatmap(hdata, ..., "
                f"resolution={sorted(completed_resolutions)[0]}, base_on='mat_EM')"
            )
