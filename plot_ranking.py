...
# ==========================================
path = "/Users/ckw/warehouse/metacell/stark/benchmark"
CSV_FILES = {}
for csv in os.listdir(path):
    name = csv.split(".")[0]
    CSV_FILES[name] = os.path.join(path, csv)
...
def plot_funkyheatmap(csv_dict):
    ...
    ax.text(
        -0.5,
        -1.2,
        "Benchmarking Performance of Metacell Methods",
        fontsize=16,
        fontweight="bold",
        ha="left",
        color="#222222",
    ...
    for i, method in enumerate(methods):
        ...
        # 底层浅灰色的背景轨
        ax.barh(
            y, bar_width_max, left=x_bar_start, height=0.35, color="#eeeeee", zorder=1
        )
        # 真实红橙色进度条代表实际综合得分
        ax.barh(
            y,
            overall_val * bar_width_max,
            left=x_bar_start,
            height=0.35,
            color="#f26b5b",
            zorder=2,
        ...
        ax.text(
            x_bar_start + overall_val * bar_width_max + 0.08,
            y,
            f"{overall_val:.2f}",
            va="center",
            fontsize=10,
