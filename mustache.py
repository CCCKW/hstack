...
# NumPy 2.0 compatibility for older cooler internals used by this script.
np.string_ = getattr(np, "string_", np.bytes_)
np.unicode_ = getattr(np, "unicode_", np.str_)
...
def read_cooler(f, distance_in_bp, chr1, chr2, cooler_balance):
    ...
    if chr1 not in clr.chromnames or chr2 not in clr.chromnames:
        raise NameError('wrong chromosome name!')
    use_balance = cooler_balance if cooler_balance else False

    if chr1 == chr2:
        result = clr.matrix(balance=use_balance, sparse=True).fetch(chr1)
        result = sparse.triu(result)
        np.nan_to_num(result.data, copy=False, nan=0, posinf=0, neginf=0)
        x = result.row
        y = result.col
        val = result.data
    else:
        result = clr.matrix(balance=use_balance, sparse=True).fetch(chr1, chr2)
